...
printf 'branch=%s\nagent=%s\nsource_commit=%s\nsource_tree=%s\nmodel=%s\nreasoning_effort=%s\nitem_count=%s\nstatus=running\nstarted_utc=%s\n' "$BRANCH" "$AGENT" '5af20b79e6fec323a2d5cb9344a6a584db1c635a' '90889393919f3f267d49c0d8544475806bcb1f96' 'gpt-5.5' 'xhigh' 2 "$STARTED" > "$META"
set +e
...
printf 'branch=%s\nagent=%s\nsource_commit=%s\nsource_tree=%s\nmodel=%s\nreasoning_effort=%s\nitem_count=%s\nstatus=%s\nstarted_utc=%s\nended_utc=%s\nexit_code=%s\n' "$BRANCH" "$AGENT" '5af20b79e6fec323a2d5cb9344a6a584db1c635a' '90889393919f3f267d49c0d8544475806bcb1f96' 'gpt-5.5' 'xhigh' 2 "$final_status" "$STARTED" "$ENDED" "$rc" > "$META"
exit "$rc"
