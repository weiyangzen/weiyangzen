...
printf 'branch=%s\nagent=%s\nsource_commit=%s\nsource_tree=%s\nmodel=%s\nreasoning_effort=%s\nitem_count=%s\nstatus=running\nstarted_utc=%s\n' "$BRANCH" "$AGENT" 'dd6c37c45c4836773497f878b8925057e9f5318c' '0fa8cd9a00b5b352ba560724bae33887982fd314' 'gpt-5.5' 'xhigh' 4 "$STARTED" > "$META"
set +e
...
printf 'branch=%s\nagent=%s\nsource_commit=%s\nsource_tree=%s\nmodel=%s\nreasoning_effort=%s\nitem_count=%s\nstatus=%s\nstarted_utc=%s\nended_utc=%s\nexit_code=%s\n' "$BRANCH" "$AGENT" 'dd6c37c45c4836773497f878b8925057e9f5318c' '0fa8cd9a00b5b352ba560724bae33887982fd314' 'gpt-5.5' 'xhigh' 4 "$final_status" "$STARTED" "$ENDED" "$rc" > "$META"
exit "$rc"
