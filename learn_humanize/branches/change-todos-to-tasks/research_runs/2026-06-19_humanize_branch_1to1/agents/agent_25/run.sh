...
printf 'branch=%s\nagent=%s\nsource_commit=%s\nsource_tree=%s\nmodel=%s\nreasoning_effort=%s\nitem_count=%s\nstatus=running\nstarted_utc=%s\n' "$BRANCH" "$AGENT" '0790d28514ab48bec2668f4ec069592872fed586' '421aa084f53b0286b04c69472f0e683788b7059e' 'gpt-5.5' 'xhigh' 4 "$STARTED" > "$META"
set +e
...
printf 'branch=%s\nagent=%s\nsource_commit=%s\nsource_tree=%s\nmodel=%s\nreasoning_effort=%s\nitem_count=%s\nstatus=%s\nstarted_utc=%s\nended_utc=%s\nexit_code=%s\n' "$BRANCH" "$AGENT" '0790d28514ab48bec2668f4ec069592872fed586' '421aa084f53b0286b04c69472f0e683788b7059e' 'gpt-5.5' 'xhigh' 4 "$final_status" "$STARTED" "$ENDED" "$rc" > "$META"
exit "$rc"
