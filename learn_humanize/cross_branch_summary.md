...
- Completed branch research count: `12`.
- Completed branches: `add-a-final-code-simplifier-after-codex-complete`, `add-careful-mode-v1.6.5`, `add-commit-plan-file-cli`, `add-gen-plan-command`, `add-shell-syntax-check-cicd`, `allow-only-cancel-to-mv-state`, `ask-codex-skill`, `ask-gemini`, `cancel-when-finalize`, `change-todos-to-tasks`, `h2-dev`, `main`.
...
```text
complete: 12
prepared_not_complete: 26
```

## Current Next Branches Needing Worker Completion

`claude/add-dependency-check-tA0P8`, `dev`, `dev-rlcr-with-swarm-team`, `do-not-block-stop-when-background-running`, `do-not-wish-coding`, `enhance-rlcr-with-review-loop`, `feature/codex-bypass-sandbox-env`, `fix-humanize-escape`, `fix-pr-loop`, `fix-too-strict-rule-and-enhance-plan-gen`, ... (26 total incomplete)
