...
- Completed branch research count: `5`.
- Completed branches: `add-a-final-code-simplifier-after-codex-complete`, `add-careful-mode-v1.6.5`, `add-commit-plan-file-cli`, `h2-dev`, `main`.
...
```text
complete: 5
prepared_not_complete: 33
```

## Current Next Branches Needing Worker Completion

`add-gen-plan-command`, `add-shell-syntax-check-cicd`, `allow-only-cancel-to-mv-state`, `ask-codex-skill`, `ask-gemini`, `cancel-when-finalize`, `change-todos-to-tasks`, `claude/add-dependency-check-tA0P8`, `dev`, `dev-rlcr-with-swarm-team`, ... (33 total incomplete)
