...
printf 'branch=%s\nagent=%s\nsource_commit=%s\nsource_tree=%s\nmodel=%s\nreasoning_effort=%s\nitem_count=%s\nstatus=running\nstarted_utc=%s\n' "$BRANCH" "$AGENT" '6b3819fad50a89fffae899b240ad1ce065c51d23' '35394e47731487305e72fba1af5fac0301816c20' 'gpt-5.5' 'xhigh' 120 "$STARTED" > "$META"
set +e
...
printf 'branch=%s\nagent=%s\nsource_commit=%s\nsource_tree=%s\nmodel=%s\nreasoning_effort=%s\nitem_count=%s\nstatus=%s\nstarted_utc=%s\nended_utc=%s\nexit_code=%s\n' "$BRANCH" "$AGENT" '6b3819fad50a89fffae899b240ad1ce065c51d23' '35394e47731487305e72fba1af5fac0301816c20' 'gpt-5.5' 'xhigh' 120 "$final_status" "$STARTED" "$ENDED" "$rc" > "$META"
exit "$rc"
